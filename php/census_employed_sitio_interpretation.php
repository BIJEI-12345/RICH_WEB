<?php
/**
 * AI na paliwanag (Filipino) para sa bilang ng employed residents bawat sitio — Groq.
 * Key: GROQ_API_KEY_CENSUS_EMPLOYED (.env).
 */
...
function census_employed_interpretation_replace_pamamahagi(string $text): string
{
    if ($text === '') {
        return '';
    }
    $pairs = [
        ...
        '/\bpamamahagi\s+ng\s+employed\b/iu' => 'pagtukoy ng populasyon ng mga employed',
...
/** Panatilihin ang newline para sa mga bullet; linisin lamang ang pahaba na espasyo bawat linya. */
function census_employed_normalize_interpretation_whitespace(string $s): string
{
    $s = str_replace(["\r\n", "\r"], "\n", $s);
    $lines = explode("\n", $s);
    $clean = [];
...
$apiKey = defined('GROQ_API_KEY_CENSUS_EMPLOYED') ? trim((string) GROQ_API_KEY_CENSUS_EMPLOYED) : '';
if ($apiKey === '') {
    echo json_encode([
        'success' => true,
        'interpretation' => '',
...
$totalEmployed = isset($data['totalEmployed']) ? (int) $data['totalEmployed'] : 0;
$rowsIn = $data['employedBySitio'] ?? null;
if (!is_array($rowsIn)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Missing employedBySitio'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (count($lines) === 0 || $totalEmployed === 0) {
    echo json_encode([
        'success' => true,
        'interpretation' => '',
        'recommendations' => '',
...
$linesText = [];
foreach ($lines as $L) {
    $linesText[] = '- Sitio / lokasyon sa census: "' . $L['sitio'] . '" — ' . $L['count'] . ' na resident na nakatala bilang employed (hanapbuhay / occupation field) roon';
}
$breakdown = implode("\n", $linesText);

$prompt = "You are summarizing barangay census analytics for the Philippines.\n";
$prompt .= "Ang datos ay tungkol sa mga CENSUSED na resident na nakatala bilang **employed** batay sa occupation / hanapbuhay sa census: kung ILAN sila at SAANG SITIO sila nakatala.\n\n";
$prompt .= "Breakdown — bilang ng employed residents sa bawat sitio na may hindi bababa sa 1:\n{$breakdown}\n\n";
$prompt .= "KABUUAN ng lahat ng employed residents sa aktibong census: {$totalEmployed}\n";
if ($sumRows !== $totalEmployed) {
    $prompt .= "Paalala: ang kabuuang {$totalEmployed} ay opisyal na total; ang partial sum ng mga sitio sa listahan ay {$sumRows}.\n";
}
$prompt .= "\nReturn STRICT JSON only: {\"interpretation\":\"...\",\"recommendations\":\"...\"}.\n";
$prompt .= "ISTRUKTURA ng interpretation (mahigpit):\n";
$prompt .= "- Una: 1-2 pangungusap na buod — kabuuang employed at konteksto ng pagtukoy ng populasyon (walang HTML).\n";
...
$prompt .= "- Bawat linya ng bullet: • [eksaktong pangalan ng sitio gaya sa datos]: [bilang] employed — isang maikling pangungusap (hal. kung mas mataas o mas mababa kumpara sa iba).\n";
...
$prompt .= "\nHiwalay na string na \"recommendations\": 2-4 na pangungusap — posibleng epekto sa lebel ng barangay (trabaho, ekonomiya, pagsasagawa ng programa) at praktikal na rekomendasyon (skills training, koordinasyon sa LGU, monitoring). Walang HTML.\n";
...
if ($response === false || $curlError !== '' || $httpCode !== 200) {
    error_log("Groq census employed sitio: HTTP {$httpCode} {$curlError}");
    echo json_encode([
        'success' => true,
        'interpretation' => '',
        'recommendations' => '',
...
$interpretation = census_employed_normalize_interpretation_whitespace((string) $interpretation);
$interpretation = mb_substr($interpretation, 0, 2500);

$interpretation = census_employed_interpretation_replace_pamamahagi($interpretation);
$interpretation = census_employed_normalize_interpretation_whitespace($interpretation);

// Kung walang bullet bawat sitio ang modelo, mag-append ng listahan mula sa parehong datos ng breakdown.
if ($interpretation !== '' && count($lines) > 0) {
    ...
    if (!$hasBulletLine) {
        $buf = [];
        foreach ($lines as $L) {
            $buf[] = '• ' . $L['sitio'] . ': ' . $L['count'] . ' na employed na resident na nakatala sa sitio na ito.';
        }
...
$recommendations = census_employed_interpretation_replace_pamamahagi($recommendations);
