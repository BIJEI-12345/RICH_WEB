<?php
/**
 * AI na paliwanag (Filipino) para sa bilang ng PWD / may disability record bawat sitio — Groq.
 * Key: GROQ_API_KEY_CENSUS_PWD (.env).
 */
...
function census_pwd_interpretation_replace_pamamahagi(string $text): string
{
    if ($text === '') {
        return '';
    }
    $pairs = [
        '/\bpamamahagi\s+ng\s+mga\s+PWD\b/iu' => 'pagtukoy ng populasyon ng mga resident na may PWD record',
        '/\bpamamahagi\s+ng\s+PWD\b/iu' => 'pagtukoy ng populasyon ng mga resident na may PWD record',
        '/\bpamamahagi\s+ng\s+mga\s+pwd\b/iu' => 'pagtukoy ng populasyon ng mga resident na may PWD record',
...
function census_pwd_normalize_interpretation_whitespace(string $s): string
{
    $s = str_replace(["\r\n", "\r"], "\n", $s);
    $lines = explode("\n", $s);
    $clean = [];
...
$apiKey = defined('GROQ_API_KEY_CENSUS_PWD') ? trim((string) GROQ_API_KEY_CENSUS_PWD) : '';
if ($apiKey === '') {
    echo json_encode([
        'success' => true,
        'interpretation' => '',
...
$totalPwd = isset($data['totalPwd']) ? (int) $data['totalPwd'] : 0;
$rowsIn = $data['pwdBySitio'] ?? null;
if (!is_array($rowsIn)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Missing pwdBySitio'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$linesText = [];
foreach ($lines as $L) {
    $linesText[] = '- Sitio / lokasyon sa census: "' . $L['sitio'] . '" — ' . $L['count'] . ' na resident na may nakatalang PWD / disability record roon (aggregate lamang; walang medikal na detalye)';
}
$breakdown = implode("\n", $linesText);

$prompt = "You are summarizing barangay census analytics for the Philippines.\n";
$prompt .= "Ang datos ay tungkol sa mga CENSUSED na resident na **may PWD o disability record** sa census (hal. disability field, PWD flag): kung ILAN ang bilang (aggregate) at SAANG SITIO sila nakatala.\n";
$prompt .= "Maging paggalang at propesyonal; lebel ng barangay/komunidad lamang — HUWAG mag-diagnose ng indibidwal, HUWAG mag-imbento ng medikal na detalye.\n\n";
$prompt .= "Breakdown — bilang ng resident na may PWD record sa bawat sitio na may hindi bababa sa 1:\n{$breakdown}\n\n";
$prompt .= "KABUUAN ng lahat ng resident na may PWD record sa aktibong census: {$totalPwd}\n";
if ($sumRows !== $totalPwd) {
    $prompt .= "Paalala: ang kabuuang {$totalPwd} ay opisyal na total; ang partial sum ng mga sitio sa listahan ay {$sumRows}.\n";
}
...
$prompt .= "\nHiwalay na \"recommendations\": 2-4 na pangungusap — posibleng pangangailangan sa suporta/inclusivity sa lebel ng barangay at praktikal na rekomendasyon (MSWD/PDAO, accessibility ng pasilidad, impormasyon sa programa, walang medikal na diagnosis). Walang HTML.\n";

$requestData = [
    'model' => 'llama-3.1-8b-instant',
    'messages' => [
        ['role' => 'system', 'content' => 'Return strict JSON with keys interpretation and recommendations. Filipino. interpretation uses \\n and bullets •. Never use pamamahagi. Respectful; community-level aggregates only; no individual medical claims.'],
...
if ($response === false || $curlError !== '' || $httpCode !== 200) {
    error_log("Groq census pwd sitio: HTTP {$httpCode} {$curlError}");
    echo json_encode([
        'success' => true,
        'interpretation' => '',
        'recommendations' => '',
...
$interpretation = census_pwd_normalize_interpretation_whitespace((string) $interpretation);
$interpretation = mb_substr($interpretation, 0, 2500);

$interpretation = census_pwd_interpretation_replace_pamamahagi($interpretation);
$interpretation = census_pwd_normalize_interpretation_whitespace($interpretation);
...
if ($interpretation !== '' && count($lines) > 0) {
    ...
    if (!$hasBulletLine) {
        $buf = [];
        foreach ($lines as $L) {
            $buf[] = '• ' . $L['sitio'] . ': ' . $L['count'] . ' na resident na may PWD record na nakatala sa sitio na ito.';
        }
...
$recommendations = census_pwd_interpretation_replace_pamamahagi($recommendations);
