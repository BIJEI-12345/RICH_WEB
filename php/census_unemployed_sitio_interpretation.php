<?php
/**
 * AI na paliwanag (Filipino) para sa bilang ng unemployed residents bawat sitio — Groq.
 * Key: GROQ_API_KEY_CENSUS_UNEMPLOYED (.env).
 */
...
function census_unemployed_interpretation_replace_pamamahagi(string $text): string
{
    if ($text === '') {
        return '';
    }
    $pairs = [
        '/\bpamamahagi\s+ng\s+mga\s+unemployed\b/iu' => 'pagtukoy ng populasyon ng mga unemployed',
        '/\bpamamahagi\s+ng\s+unemployed\b/iu' => 'pagtukoy ng populasyon ng mga unemployed',
...
function census_unemployed_normalize_interpretation_whitespace(string $s): string
{
    $s = str_replace(["\r\n", "\r"], "\n", $s);
    $lines = explode("\n", $s);
    $clean = [];
...
$apiKey = defined('GROQ_API_KEY_CENSUS_UNEMPLOYED') ? trim((string) GROQ_API_KEY_CENSUS_UNEMPLOYED) : '';
if ($apiKey === '') {
    echo json_encode([
        'success' => true,
        'interpretation' => '',
...
$totalUnemployed = isset($data['totalUnemployed']) ? (int) $data['totalUnemployed'] : 0;
$rowsIn = $data['unemployedBySitio'] ?? null;
if (!is_array($rowsIn)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Missing unemployedBySitio'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (count($lines) === 0 || $totalUnemployed === 0) {
    echo json_encode([
        'success' => true,
        'interpretation' => '',
        'recommendations' => '',
...
$linesText = [];
foreach ($lines as $L) {
    $linesText[] = '- Sitio / lokasyon sa census: "' . $L['sitio'] . '" — ' . $L['count'] . ' na resident na nakatala bilang unemployed (occupation / hanapbuhay) roon';
}
$breakdown = implode("\n", $linesText);

$prompt = "You are summarizing barangay census analytics for the Philippines.\n";
$prompt .= "Ang datos ay tungkol sa mga CENSUSED na resident na nakatala bilang **unemployed** batay sa occupation / hanapbuhay sa census: kung ILAN sila at SAANG SITIO sila nakatala.\n\n";
$prompt .= "Breakdown — bilang ng unemployed residents sa bawat sitio na may hindi bababa sa 1:\n{$breakdown}\n\n";
$prompt .= "KABUUAN ng lahat ng unemployed residents sa aktibong census: {$totalUnemployed}\n";
if ($sumRows !== $totalUnemployed) {
    $prompt .= "Paalala: ang kabuuang {$totalUnemployed} ay opisyal na total; ang partial sum ng mga sitio sa listahan ay {$sumRows}.\n";
}
$prompt .= "\nReturn STRICT JSON only: {\"interpretation\":\"...\",\"recommendations\":\"...\"}.\n";
$prompt .= "ISTRUKTURA ng interpretation (mahigpit):\n";
$prompt .= "- Una: 1-2 pangungusap na buod — kabuuang unemployed at konteksto ng pagtukoy ng populasyon (walang HTML).\n";
...
$prompt .= "- Bawat linya ng bullet: • [eksaktong pangalan ng sitio gaya sa datos]: [bilang] unemployed — isang maikling pangungusap.\n";
...
$prompt .= "- Sa JSON string ng \"interpretation\", gumamit ng \\n pagitan ng buod at ng lista ng bullets.\n";
$prompt .= "- Huwag mag-imbento ng numero o pangalan ng sitio na wala sa datos.\n";
$prompt .= "\nHiwalay na \"recommendations\": 2-4 na pangungusap — posibleng epekto (kabuhayan, panganib sa komunidad) at rekomendasyon (livelihood, job fair, skills training, referral sa LGU/PUES). Walang HTML.\n";
...
if ($response === false || $curlError !== '' || $httpCode !== 200) {
    error_log("Groq census unemployed sitio: HTTP {$httpCode} {$curlError}");
    echo json_encode([
        'success' => true,
        'interpretation' => '',
        'recommendations' => '',
...
$interpretation = census_unemployed_normalize_interpretation_whitespace((string) $interpretation);
$interpretation = mb_substr($interpretation, 0, 2500);

$interpretation = census_unemployed_interpretation_replace_pamamahagi($interpretation);
$interpretation = census_unemployed_normalize_interpretation_whitespace($interpretation);
...
if ($interpretation !== '' && count($lines) > 0) {
    ...
    if (!$hasBulletLine) {
        $buf = [];
        foreach ($lines as $L) {
            $buf[] = '• ' . $L['sitio'] . ': ' . $L['count'] . ' na unemployed na resident na nakatala sa sitio na ito.';
        }
...
$recommendations = census_unemployed_interpretation_replace_pamamahagi($recommendations);
